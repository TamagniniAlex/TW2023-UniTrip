...
if (dataIsCorrect()) {
    ...
    $count = $register->model->check_user($nickname, $mail);

    if ($count > 0) {
        header('location: ..//view/index.html?error=1');
    } else {
        $register->model->insert_user($nickname, $password, $mail, $name, $surname, $photo_url, $description, $birth_date, $join_date);
    }
